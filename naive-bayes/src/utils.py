...
def softmax(x, axis=1):
    ...
    softmax = np.exp(x - np.max(x, axis=axis, keepdims=True)) / np.sum(np.exp(x - np.max(x, axis=axis, keepdims=True)), axis=axis, keepdims=True)
    return softmax
...
def stable_log_sum(X):
    ...
    if np.min(X) > -743: 
         # if all elements of X are greater than -743, tiis is equal to 
