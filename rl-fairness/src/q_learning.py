    ...
    def fit(self, env, steps=1000, num_bins=100):
        ...
        for i in range(steps):
            ...
            if choice < self.epsilon: 
                max_args = np.argwhere(state_action_values[current_state,:]== np.amax(state_action_values[current_state,:]))
            ...
            # a is random max index of state_action_values[R[0],:]
            max_args = np.argwhere(state_action_values[R[0],:]== np.amax(state_action_values[R[0],:]))
            a = src.random.choice(max_args.flatten())

            state_action_values[current_state,action] = state_action_values[current_state,action] + self.alpha * (R[1] + self.gamma * state_action_values[R[0], a] - state_action_values[current_state,action])
            current_state = R[0]
            curr_rewards = np.append(curr_rewards, R[1])

            # if terminated or truncated, reset the environment (update current state)
            if R[2] or R[3]:
              current_state, _ = env.reset()
