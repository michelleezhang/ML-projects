...
def replace_nonfinite_in_place(x):
    ...
    """
    np.place(x, np.logical_not(np.isfinite(x)), [0])    # np.place changes elements of an array based on conditional and input values
...
def replace_nans_out_of_place(x):
    ...
    """
    return np.where(np.isnan(x), [0], x)    # returns array where elements that are nan are replaced w 0, otherwise element is from x 
...
def find_mode(x):
    ...
    Returns:
        the mode of x
    """
    numbers, counts = np.unique(x, return_counts=True)  # np.unique w ret_counts=True returns vector of unique vals and vector of corresponding counts of each val
...
def flip_and_slice_matrix(x):
    ...
    Returns:
      a numpy matrix
    """
    return np.fliplr(x[::3]) # pick out every third row (x = x[::3]) and flip matrix horizontally (np.fliplr)
...
def divide_matrix_along_rows(x, y):
    ...
    Returns:
      a numpy matrix
    """
    y = y[:, np.newaxis]    # reshape the size of y to (len_y, 1) so it can be divided with x
    return x/y