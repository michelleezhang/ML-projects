...
def train_test_split(features, labels, fraction):
    ...
    if fraction == 1.0:
        ...
        # we just want to return the first M rows as training and the remaining N - M rows as testing
        N = features.shape[0]          # number of rows in features (shape returns [num rows, num cols])
        M = int(features.shape[0] * fraction)   

        train_features = features[:M, :]    # first M rows 
        train_labels = labels[:M]           
        test_features = features[M:N, :]    # next N rows (after M)
        test_labels = labels[M:N]           
...
def cross_validation(features, labels, n_folds):
    ...
    # goal is to split features and labels into n_folds groups that divide the data as evenly as possible
    # for each group, return a tuple that treats that group as the test set and all other groups combine to make the training set
    featuregroups = np.array_split(features, n_folds)
    labelgroups = np.array_split(labels, n_folds)

    result = []

    for i in range(n_folds): 
        # the i-th subarray is the test slice
        test_features = featuregroups[i]
        test_labels = labelgroups[i]

        # deletes i-th subarray, then combines them all together to get the train slice
        train_features = np.row_stack(np.delete(featuregroups, i, 0))
        train_labels = np.row_stack(np.delete(labelgroups, i, 0)) 

        tup = (train_features, train_labels, test_features, test_labels)
        result.append(tup)
